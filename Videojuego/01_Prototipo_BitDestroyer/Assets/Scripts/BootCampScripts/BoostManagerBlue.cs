...
- Description:
    ...
    a hand list cards of the player blue. It also apply a boost of three point according
    to selected card.
*/
...
public class BoostManagerBlue : MonoBehaviour
{
    // Panel to instantiate the buttons
    public GameObject boostPanelBlue;
    ...
    void Start()
    {
        ...
        boostPanelBlue.SetActive(false);
    ...
    private void InstantiateBoostButtons() {
        ...
        if (defenseCardsList.Count > 0) {
            foreach (DefenseCard defenseCard in defenseCardsList) {
                ...
                GameObject boostButtonObj = Instantiate(boostButtonPrefab, boostPanelBlue.transform);
